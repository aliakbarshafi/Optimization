...
// sum(over q) x[p][q] <= (sum(over q)d[q])*y[p]  for all p belong to P
// 
...
int main(int argc, char **argv)
{
	...
	try {
		...
		*/
		...
		// sum(over q) x[p][q] <= (sum(over q)d[q])*y[p]  for all p belong to P		// 
		for (p = 0; p < n_p; p++) {
			IloExpr upperBound(env);
			IloNum demandSum = 0;
			for (q = 0; q < n_q; q++) {
				demandSum += d[q];
				upperBound += x[p][q];
			}
			mod.add(upperBound <= demandSum * y[p]);
			modLP.add(upperBound <= demandSum * y[p]);
			upperBound.end();
		}
		
		// Attaching a cplex object to solve MIP instance
		IloCplex cplex(mod);
		cplex.setParam(IloCplex:: ClockType, 1);
		env.out() << endl << "*****************************    Section A   ******************************"  << endl << endl;
		cplex.solve();

		// Relaxing the binary constraint on y for LP Relaxation and attaching a cplex object to solve LP relaxation
		modLP.add(IloConversion(env, y, ILOFLOAT));
		IloCplex cplexLP(modLP);
		cplexLP.setParam(IloCplex::ClockType, 1);
		cplexLP.solve();

		env.out() << endl << "Facility : Total Production" << endl;
		IloInt i, j;
		for (i = 0; i < n_p; i++) {
		...
		env.out() << endl << "Percentage Gap of MIP and LP Solutions: " << 100 * (cplex.getObjValue() - cplexLP.getObjValue()) / cplex.getObjValue() << endl ;
		...
		// Creating cplex solvers for studying the effecs of turning off the cuts 
		IloCplex cplexCutOffMIP(mod);
		IloCplex cplexCutOffLP(modLP);
		cplexCutOffMIP.setParam(IloCplex::ClockType, 1);
		cplexCutOffLP.setParam(IloCplex::ClockType, 1);

		//Setting default cuts OFF for cplex and cplexLP */
		cplexCutOffMIP.setParam(IloCplex::EachCutLim, 0);
		cplexCutOffLP.setParam(IloCplex::EachCutLim, 0);

		env.out() << endl << "******************************    Section B   ******************************" << endl<<endl;
		

		/*Turn off pre-processing*/

		//cplexCutOffMIP.setParam(IloCplex::PreInd, 0);
		//cplexCutOffMIP.setParam(IloCplex::PreInd, 0);

		/* Node selection Strategies
		 Depth First Search: CPX_NODESEL_DFS
		 Best Bound Search: CPX_NODESEL_BESTBOUND
		*/

		//cplexCutOffMIP.setParam(IloCplex::NodeSel, CPX_NODESEL_DFS);
		//cplexCutOffMIP.setParam(IloCplex::NodeSel, CPX_NODESEL_BESTBOUND);
		//cplexCutOffLP.setParam(IloCplex::NodeSel, CPX_NODESEL_DFS);
		//cplexCutOffLP.setParam(IloCplex::NodeSel, CPX_NODESEL_BESTBOUND);

		/* MIP variable selection strategies
		The minimum infeasibility rule: CPX_VARSEL_MININFEAS
		The maximum infeasibility rule: CPX_VARSEL_MAXINFEAS
		*/

		//cplexCutOffMIP.setParam(IloCplex::VarSel, CPX_VARSEL_MININFEAS);
		//cplexCutOffMIP.setParam(IloCplex::VarSel, CPX_VARSEL_MAXINFEAS);
		//cplexCutOffLP.setParam(IloCplex::VarSel, CPX_VARSEL_MININFEAS);
		//cplexCutOffLP.setParam(IloCplex::VarSel, CPX_VARSEL_MAXINFEAS);

		cplexCutOffMIP.solve();
		cplexCutOffLP.solve();
		
		env.out() << endl << "Optimal Objective Value of MIP instance: " << cplexCutOffMIP.getObjValue() << endl;
		env.out() << endl << "No of Integer Variables: " << n_p << endl;
		env.out() << endl << "No of Continuous Variables: " << n_p*n_q << endl;
		env.out() << endl << "No of Constraints: " << cplexCutOffMIP.getNrows() << endl;
		env.out() << endl << "Run time to solve the LP relaxation: " << cplexCutOffLP.getTime() << endl;
		env.out() << endl << "Optimal objective function value for this LP relaxation: " << cplexCutOffLP.getObjValue() << endl;
		env.out() << endl << "Run Time to solve MIP: " << cplexCutOffMIP.getTime() << endl;
		env.out() << endl << "No of Nodes: " << cplexCutOffMIP.getNnodes() << endl;
		env.out() << endl << "Percentage Gap pf MIP and LP Solutions: " << 100 * (cplexCutOffMIP.getObjValue() - cplexCutOffLP.getObjValue()) / cplexCutOffMIP.getObjValue() << endl;
		env.out() << endl << "No of Cuts: " << cplexCutOffMIP.getNcuts(IloCplex::CutMir) +
			cplexCutOffMIP.getNcuts(IloCplex::CutImplBd) +
			cplexCutOffMIP.getNcuts(IloCplex::CutFlowCover) +
			cplexCutOffMIP.getNcuts(IloCplex::CutFlowPath) << endl;
	}
